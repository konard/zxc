...
idx::define_index! {
    #[derive(Debug)]
    pub struct PreorderIndex = u32;
}

impl PreorderIndex {
    #[inline]
    pub fn try_from_usize(n: usize) -> Option<Self> {
        if n <= Self::MAX_INDEX { Some(Self::new(n)) } else { None }
    }

    #[inline]
    pub fn checked_add(self, other: Self) -> Option<Self> {
        self._raw.checked_add(other._raw).map(Self::from_raw)
    }

    #[inline]
    pub fn checked_sub(self, other: Self) -> Option<Self> {
        self._raw.checked_sub(other._raw).map(Self::from_raw)
    }
}
...
impl Step for PreorderIndex {
    ...
    fn forward_checked(start: Self, n: usize) -> Option<Self> {
        Self::try_from_usize(n).and_then(|n| start.checked_add(n))
    }

    #[inline]
    fn backward_checked(start: Self, n: usize) -> Option<Self> {
        Self::try_from_usize(n).and_then(|n| start.checked_sub(n))
    }
}

// Inspired from:
//  https://doc.rust-lang.org/stable/nightly-rustc/src/rustc_data_structures/graph/dominators/mod.rs
